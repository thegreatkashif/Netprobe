...
async def _check_host(host, semaphore, ping_timeout, tcp_timeout):
    async with semaphore:
        if await _ping_alive(host, timeout=ping_timeout):
            return host

        if await _tcp_probe(host, timeout=tcp_timeout):
            return host
